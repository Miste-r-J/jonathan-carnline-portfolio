...
from trading_system.runtime_engine.premarket_planner.bot import PlannerTargetPublishError, PremarketPlannerBot
...
from trading_system.runtime_engine.premarket_planner.runtime import build_readiness_report
...
async def test_dispatch_partial_success_when_one_target_forbidden(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("trading_system.runtime_engine.premarket_planner.bot.build_plan_payload", lambda config, target: _FakePayload())
...
async def test_periodic_dispatch_does_not_raise_when_dispatch_fails(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("trading_system.runtime_engine.premarket_planner.bot.datetime", SimpleNamespace(
...
""".strip(),
    ...
    monkeypatch.setattr(
        "trading_system.runtime_engine.premarket_planner.runtime.probe_discord_token",
        lambda token: {"status": "ok", "detail": "http 200"},
    )
    monkeypatch.setattr(
        "trading_system.runtime_engine.premarket_planner.runtime.build_auth_diagnostics",
