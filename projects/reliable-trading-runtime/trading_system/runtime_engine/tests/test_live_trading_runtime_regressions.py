...
from trading_system.runtime_engine.integrations.cli import live_trading_runtime as live_trading_runtime_mod
...
from trading_system.development_tools.audit_live_run import audit_run
...
def test_apply_preset_to_args_maps_phase2_force_open_thresholds() -> None:
    parser = live_trading_runtime_mod._build_arg_parser()
    argv = ["--preset", "es_prop_safe_v1"]
    args = parser.parse_args(argv)

    applied = live_trading_runtime_mod._apply_preset_to_args(args, argv)
...
def test_count_unique_bar_days_from_fiber_rows() -> None:
    ...
    assert live_trading_runtime_mod._count_unique_bar_days(rows) == 2


def test_adjusted_v4_preset_allowed_for_paper_and_demo_live_only() -> None:
    paper_allowed = live_trading_runtime_mod._approved_preset_names_for_exec_policy("paper")
    ...
    demo_live_allowed = live_trading_runtime_mod._approved_preset_names_for_exec_policy("live", demo_args)
    ...
    funded_live_allowed = live_trading_runtime_mod._approved_preset_names_for_exec_policy("live", funded_args)
...
def test_adjusted_v4_preset_synthesizes_entries_only_in_live() -> None:
    ...
    applied = live_trading_runtime_mod._apply_preset_to_args(parser.parse_args(argv), argv)
...
def test_stream_source_does_not_use_shelf_arm_force_exit_token() -> None:
    source = Path(live_trading_runtime_mod.__file__).read_text(encoding="utf-8")
...
def _make_streamer(tmp_path: Path) -> LiveCSVStreamer:
    ...
    streamer._get_build_info = lambda: {
        ...
        "package_path": "live_trading_runtime.py",
    }
    ...
    streamer.state_stream_csv.write_text(",".join(live_trading_runtime_mod.STATE_CSV_COLUMNS) + "\n", encoding="utf-8")
    streamer.signal_to_order_path.write_text("", encoding="utf-8")
    streamer.lifecycle_events_csv_path.write_text(",".join(live_trading_runtime_mod.LIFECYCLE_EVENT_CSV_COLUMNS) + "\n", encoding="utf-8")
...
def test_policy_guard_does_not_count_backfill_candidates(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(live_trading_runtime_mod.policy_adapter, "guard_loaded", lambda: True)
    ...
    monkeypatch.setattr(live_trading_runtime_mod.policy_adapter, "pretrade_gate", _unexpected_pretrade)
...
def test_update_trend_mode_bootstraps_default_policy_in_live(tmp_path: Path) -> None:
    ...
    assert isinstance(streamer.policy, live_trading_runtime_mod.EntryPolicyConfig)
...
def test_activate_preset_syncs_runtime_regime_policy_and_status_fields(tmp_path: Path) -> None:
    ...
    original = live_trading_runtime_mod.PRESETS.get(preset_name)
    live_trading_runtime_mod.PRESETS[preset_name] = {
    ...
    try:
        ...
        if original is None:
            live_trading_runtime_mod.PRESETS.pop(preset_name, None)
        else:
            live_trading_runtime_mod.PRESETS[preset_name] = original
...
def test_activate_same_preset_applies_hold_cap(tmp_path: Path) -> None:
    ...
    original = live_trading_runtime_mod.PRESETS.get(preset_name)
    live_trading_runtime_mod.PRESETS[preset_name] = {"max_hold_bars": 12}
    try:
        streamer.active_preset_name = preset_name
        streamer.active_preset_cfg = live_trading_runtime_mod.PRESETS[preset_name]
...
def test_normalize_trade_window_for_tod_forces_24h_even_when_cli_window_forced() -> None:
    ...
    meta = live_trading_runtime_mod._normalize_trade_window_for_tod(args, ["--gate_tod", "--trade_window_start", "07:00", "--trade_window_end", "14:00"])
...
def test_fiber_server_eviction_prefers_nonbar_first() -> None:
    srv = live_trading_runtime_mod.FiberServer("127.0.0.1", 0, queue_max=3, evict_batch=1, drop_priority="nonbar_first")
...
def test_fiber_server_enqueue_retries_after_eviction() -> None:
    srv = live_trading_runtime_mod.FiberServer("127.0.0.1", 0, queue_max=2, evict_batch=1, drop_priority="oldest")
    srv.queue.put({"type": "BAR", "utcTicks": 1})
    srv.queue.put({"type": "BAR", "utcTicks": 2})
...
def test_snapshot_stale_recovery_suppresses_hard_reset_when_ts_invalid(tmp_path: Path) -> None:
    ...
    printed: list[str] = []
    original_safe_print = live_trading_runtime_mod._safe_print
    live_trading_runtime_mod._safe_print = lambda msg: printed.append(str(msg))
    ...
    try:
        ...
        live_trading_runtime_mod._safe_print = original_safe_print
...
def test_pre_send_entry_guard_uses_lag_adjusted_age(tmp_path: Path) -> None:
    ...
    streamer.bar_interval_sec = 300.0
    streamer.session_tz = "America/Denver"
    intent = live_trading_runtime_mod.ExecutionIntent(
        intent_id="cid-1",
        action="OPEN",
        side="SHORT",
        qty=1,
    ...
    original_now = live_trading_runtime_mod._now_denver
    live_trading_runtime_mod._now_denver = lambda: pd.Timestamp("2026-05-05T06:16:38-06:00")
    try:
        violation = streamer._pre_send_entry_guard_violation(intent)
    finally:
        live_trading_runtime_mod._now_denver = original_now
...
def test_phase_lag_hysteresis_keeps_live_during_single_window_with_heartbeat() -> None:
    phase, confirmations, current_ok = live_trading_runtime_mod._next_phase_state(
        lag_sec=275.0,
...
def test_feature_warmup_block_preserves_status_last_bar_ts(tmp_path: Path) -> None:
    ...
    fiber_bar_processed_ts = live_trading_runtime_mod._canonical_ts_str(bar_ts)
...
def test_parity_preset_inherited_slippage_override_accepts_live_market_fill(
    ...
    class _GuardStub:
        def __init__(self) -> None:
            self.config = live_trading_runtime_mod.EnhancedGuardRailConfig(
    ...
    monkeypatch.setitem(live_trading_runtime_mod.PRESETS, "base_slip_6", {"max_slippage_points": 6.0})
    streamer = _make_streamer(tmp_path)
    streamer.disable_risk = False
    streamer.tick_size = 0.25
...
def test_prop_closed_trade_uses_canonical_trade_row_when_open_trade_entry_is_contaminated(tmp_path: Path) -> None:
    ...
    streamer.trades_csv = tmp_path / "trades.csv"
    with streamer.trades_csv.open("w", encoding="utf-8", newline="\n") as fh:
        writer = csv.DictWriter(fh, fieldnames=live_trading_runtime_mod.TRADES_HEADER_FIELDS, lineterminator="\n")
        writer.writeheader()
        writer.writerow(
            {
...
def test_simulated_replay_fill_upserts_existing_open_trade_row(tmp_path: Path) -> None:
    ...
    with streamer.trades_csv.open("w", encoding="utf-8", newline="\n") as fh:
        writer = csv.DictWriter(fh, fieldnames=live_trading_runtime_mod.TRADES_HEADER_FIELDS, lineterminator="\n")
        writer.writeheader()
        row = {field: "" for field in live_trading_runtime_mod.TRADES_HEADER_FIELDS}
        row.update(
            {
...
def test_finalize_replay_trades_csv_closes_open_rows_and_merges_sim_duplicates(tmp_path: Path) -> None:
    ...
    with streamer.trades_csv.open("w", encoding="utf-8", newline="\n") as fh:
        writer = csv.DictWriter(fh, fieldnames=live_trading_runtime_mod.TRADES_HEADER_FIELDS, lineterminator="\n")
        writer.writeheader()
        base = {field: "" for field in live_trading_runtime_mod.TRADES_HEADER_FIELDS}
        row_a = dict(base)
        row_a.update(
            {
...
def test_no_trade_reason_helper_formats_publish_block_taxonomy() -> None:
    reason = live_trading_runtime_mod._resolve_no_trade_reason_detail(
        action="NO_TRADE",
...
def test_pre_arm_close_model_event_is_suppressed(tmp_path: Path) -> None:
    ...
    orig_align = live_trading_runtime_mod._align_X
    orig_predict = live_trading_runtime_mod._predict_proba_safely
    orig_cal = live_trading_runtime_mod.calibrate_proba
    try:
        live_trading_runtime_mod._align_X = lambda frame, expected, warn_context=None, strict=True: (frame[list(expected)], None)
        live_trading_runtime_mod._predict_proba_safely = lambda *args, **kwargs: [0.95]
        live_trading_runtime_mod.calibrate_proba = lambda raw, cal: raw
        ...
        live_trading_runtime_mod._align_X = orig_align
        live_trading_runtime_mod._predict_proba_safely = orig_predict
        live_trading_runtime_mod.calibrate_proba = orig_cal
...
def test_phase2_close_missing_features_is_soft_skipped(tmp_path: Path) -> None:
    ...
    orig_align = live_trading_runtime_mod._align_X
    try:
        live_trading_runtime_mod._align_X = lambda *args, **kwargs: (_ for _ in ()).throw(
...
def test_position_snapshot_order_update_repair_preserves_pnl_shelf_state(tmp_path: Path) -> None:
    streamer = _make_streamer(tmp_path)
    streamer.guardrail_config = live_trading_runtime_mod.GuardRailConfig()
...
def test_guardrail_price_age_missing_does_not_disarm_fresh_protected_snapshot(tmp_path: Path) -> None:
    ...
    guard = live_trading_runtime_mod.evaluate_guardrails(ctx, streamer.guardrail_config, preflight=True)
...
def test_flip_on_flat_normalizes_to_open(tmp_path: Path) -> None:
    ...
    flip_intent = live_trading_runtime_mod.ExecutionIntent(
    ...
    captured: list[live_trading_runtime_mod.ExecutionIntent] = []
...
def test_close_suppressed_when_flat_and_snapshot_stale_reconcile(tmp_path: Path) -> None:
    ...
    close_intent = live_trading_runtime_mod.ExecutionIntent(
...
def test_lineage_entry_exit_isolation_keeps_flip_open_eligible(tmp_path: Path) -> None:
    ...
    open_intent = live_trading_runtime_mod.ExecutionIntent(
...
def test_lineage_autoheal_allows_flip_open_once_from_matching_close_chain(tmp_path: Path) -> None:
    ...
    intent2 = live_trading_runtime_mod.ExecutionIntent(
...
def test_strict_intent_parity_hard_block_classifier() -> None:
    assert live_trading_runtime_mod._strict_intent_parity_is_hard_block("snapshot_stale")
    assert live_trading_runtime_mod._strict_intent_parity_is_hard_block("account_unauthorized")
    assert not live_trading_runtime_mod._strict_intent_parity_is_hard_block("setup")
    assert not live_trading_runtime_mod._strict_intent_parity_is_hard_block("phase2_setup_blocked")
...
def test_close_intent_allows_missing_stop_target_pre_send(tmp_path: Path) -> None:
    ...
    streamer._ensure_nt_protocol_context = lambda *args, **kwargs: True
    streamer._validate_nt_order_contract = live_trading_runtime_mod.LiveCSVStreamer._validate_nt_order_contract.__get__(
        streamer, live_trading_runtime_mod.LiveCSVStreamer
    )
...
def test_health_summary_ignores_planned_only_no_fill_rows_for_execution_warning(tmp_path: Path) -> None:
    ...
    streamer.state.position_state = "FLAT"
    streamer.trades_csv = tmp_path / "trades.csv"
    row = {k: "" for k in live_trading_runtime_mod.TRADES_HEADER_FIELDS}
    row.update(
        {
