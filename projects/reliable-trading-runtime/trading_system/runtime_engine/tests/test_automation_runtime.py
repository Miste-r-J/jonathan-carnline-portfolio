...
from trading_system.runtime_engine.automation_runtime import AUTOMATION_COMMANDS
from trading_system.runtime_engine.integrations.route_config import resolve_automation_discord_token_details
from trading_system.runtime_engine.premarket_planner.bot import PremarketPlannerBot
from trading_system.runtime_engine.premarket_planner.config import load_config
from trading_system.runtime_engine.premarket_planner.news import build_news_summary
...
def test_automation_readiness_includes_news_source(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("OPENCLAW_DISCORD_CONFIG", str(config_path))
    monkeypatch.setattr(
        "trading_system.runtime_engine.premarket_planner.runtime._safe_json_request",
