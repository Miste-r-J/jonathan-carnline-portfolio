...
def resolve_model(spec_or_path: str) -> ModelSpec:
    ...
    # may resolve to `<repo>\artifacts` instead of `<repo>\standalone\artifacts`.
    # Add deterministic fallbacks rooted at the standalone package tree.
    try:
        trading_system_root = Path(__file__).resolve().parents[2]
        candidate_paths.append((trading_system_root / raw).resolve())
        if "\\" in raw or "/" in raw:
            candidate_paths.append((trading_system_root / "artifacts" / Path(raw).name).resolve())
    except Exception:
        pass
...
def _handle_missing_fuser(exc: Exception) -> bool:
    ...
    try:
        trainer_mod = importlib.import_module("trading_system.runtime_engine.modeling.train_hgb_multi")
