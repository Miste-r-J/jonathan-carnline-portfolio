...
def main(argv: list[str] | None = None) -> None:
    ...
    mod = importlib.import_module("trading_system.runtime_engine.integrations.cli.live_trading_runtime")
    print(f"[trading-runtime] using runtime engine: {importlib.import_module('trading_system.runtime_engine').__file__}")
    print(f"[trading-runtime] using live runtime: {mod.__file__}")

    run = getattr(mod, "main", None)
    if not callable(run):
        raise SystemExit("trading_system.runtime_engine.integrations.cli.live_trading_runtime.main not found")
    run(argv)
