"""
Minimal runtime app-config dataclasses used by the simplified gym.

This mirrors the public surface of ``na.config.app_config`` so callers that
expect the richer repo keep working, but the actual structure is intentionally
lean: only the fields referenced by the simplified ``na.bot`` modules are kept.
"""
