"""
Console-friendly metrics emitters for the simplified streamer.

The production bot writes to structured telemetry topics; here we only need a
tiny wrapper so ``stream_live_csv`` can keep calling the same helpers without
...
logger = logging.getLogger("na.metrics_log")
