...
import na.discord_addons.cli.stream_live_csv as stream_live_csv_module
...
def test_phase2_close_requires_085_threshold(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(stream_live_csv_module, "_predict_proba_safely", lambda *args, **kwargs: [0.84])
    row = pd.Series(
        {
...
def test_phase2_close_suppressed_when_profit_overlay_is_managing_trade(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(stream_live_csv_module, "_predict_proba_safely", lambda *args, **kwargs: [0.90])
    row = pd.Series(
        {
