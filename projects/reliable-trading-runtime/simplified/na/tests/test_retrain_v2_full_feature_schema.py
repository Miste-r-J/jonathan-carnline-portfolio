...
def _synthetic_ohlcv(rows: int = 500, *, seed: int = 0) -> pd.DataFrame:
    from na.bot.config import CLOSE_COL, HIGH_COL, LOW_COL, OPEN_COL, VOLUME_COL
...
def test_feature_hash_matches_manifest() -> None:
    ...
    manifest_path = Path("simplified/artifacts/phase2/candidates/retrain_v2_full/manifest.json")
...
def test_known_training_config_issues_documented() -> None:
    ...
    from na.bot.feature_constants import KNOWN_TRAINING_CONFIG_ISSUES
...
def test_ret_1_not_overwritten_by_flow() -> None:
    from na.bot.config import CLOSE_COL
    from na.bot.features import build_features
    from na.bot.features_flow_ohlcv import add_flow_ohlcv_features
...
def test_config_validation_raises_on_flow_disabled() -> None:
    from na.bot.exceptions import ConfigurationError
...
def test_manifest_paths_are_relative() -> None:
    ...
    manifest = json.loads(
        Path("simplified/artifacts/phase2/candidates/retrain_v2_full/manifest.json").read_text(encoding="utf-8")
    )
