...
Generates the morning ES summary using intraday CSV data and emits a Discord-ready embed. The planner reads `/etc/na/premarket_planner.yaml` (override with `--config`) and produces a single webhook payload; no Discord bot token is required.

## Usage

```bash
venv/bin/python -m na.premarket_planner.cli --config /etc/na/premarket_planner.yaml
```
...
ExecStart=/home/ubuntu/Desktop/venv/bin/python -m na.premarket_planner.cli --config /etc/na/premarket_planner.yaml
```
